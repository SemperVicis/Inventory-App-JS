...
class CategoryView {
  ...
  addNewCategory() {
    ...
    const savedCategories = _storage["default"].getCategories();
    const normalizedTitle = this.normalizeCategoryTitle(title);
    const existedItem = savedCategories.find((category) => this.normalizeCategoryTitle(category && category.title) === normalizedTitle);
